...
#include "MMC_Default.h"

double MMC_Default::random()
{
...
*/
double MMC_Default::sampleUniform(double min, double max)
{
 ...
 * variable using the inverse transformation technique.
*/
double MMC_Default::sampleExponential(double mean)
{
    if (mean < 0)
        return 0;
...
*/
double MMC_Default::sampleErlang(double mean, int M)
{
    if (mean < 0 || M <= 0)
        return 0;
...
double MMC_Default::sampleNormal(double mean, double stddev)
{
    double W;
    double U1, U2;
...
double MMC_Default::sampleGammaJonk(double alpha)
{
...
double MMC_Default::sampleGamma(double mean, double alpha)
{
    if (mean <= 0 || alpha <= 0)
        return 0;
...
double MMC_Default::sampleBeta(double alpha, double beta, double infLimit, double supLimit)
{
...
*/
double MMC_Default::sampleWeibull(double alpha, double scale)
{
    if (alpha <= 0 || scale <= 0)
        return 0;
...
*/
double MMC_Default::sampleLogNormal(double mean, double stddev)
{
    if (mean <= 0 || stddev <= 0)
        return 0;
...
*/
double MMC_Default::sampleTriangular(double min, double mode, double max)
{
...
double MMC_Default::sampleDiscrete(double value, double acumProb, ...)
{
    va_list args;
...
void MMC_Default::setRNGparameters(MMC_Default::RNG_Parameters* param)
{
//    seed = param->seed;
    // module = param.module;
...
MMC_Default::RNG_Parameters* MMC_Default::getRNGparameters() const
{
    return nullptr; //, module, multiplier};
}
